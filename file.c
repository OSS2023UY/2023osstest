...
void saveList(int mm) {
    ...
    char besti[100];
    if(max == bestMenu[0]) strcpy(besti,"ChickenSkewer");
    else if(max ==bestMenu[1]) strcpy(besti,"Ramen");
    else if(max ==bestMenu[2]) strcpy(besti,"Gimbob");
    else if(max ==bestMenu[3]) strcpy(besti,"Tteokbokki");


    fprintf(fp, "!%d월! \n총매출은 %d입니다.\n팔린음식의 개수는 %d개 입니다. \n가장 많이 팔린 음식은 %s입니다.", mm, totalMoney, totalCount, besti);
